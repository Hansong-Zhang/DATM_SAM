...
from utils.utils_sam import Logger, generate_cams, generate_masks, select_masks
# import wandb
import copy
import random
...
def main(args):
    save_dir = os.path.join(args.save_dir, "saved_files", "{}_res{}".format(args.subset, args.res), "ipc{}_selp{}".format(args.ipc, args.sel_p))
    mask_dir = os.path.join(args.save_dir, "saved_masks", "{}_res{}".format(args.subset, args.res), "ipc{}".format(args.ipc)) # avoid generating masks for the same ipc
    ...
    label_syn_sam = copy.deepcopy(label_syn)
    ...
    ##########################################################################################################################
    #                 generate foreground for synthetic images                                                               #
    ##########################################################################################################################    
    if args.zca:
        image_syn_ori = copy.deepcopy(image_syn).to(args.device)
        image_syn_ori = args.zca_trans.inverse_transform(image_syn_ori)
        image_syn_ori.cpu()
    else:
        image_syn_ori = copy.deepcopy(image_syn).cpu()
    image_syn_preprocess = copy.deepcopy(image_syn).cpu()

    Temp_net = get_network(args.model, channel, num_classes, im_size, dist=False).to(device)  # get a random model
    if args.model == 'ConvNetD5':
        targetlayer = Temp_net.features[16]
    else:
        pass

    expert_param = torch.load(os.path.join(expert_dir, "replay_buffer_0.pt"))[0][-1]
    for i, p in enumerate(Temp_net.parameters()):
        p.data = expert_param[i]
    Temp_net.eval()
    if not os.path.exists(os.path.join(mask_dir, "masks.pth")):
        all_masks = generate_masks(syn_imgs_ori=image_syn_ori, sam_path=args.sam_path)
        torch.save(all_masks, os.path.join(mask_dir, "masks.pth"))
    else:
        all_masks = torch.load(os.path.join(mask_dir, "masks.pth"))
        print("--> masks already exist, loaded")
    if not os.path.exists(os.path.join(mask_dir, "gen_cams.pth")):
        gen_cams = generate_cams(syn_imgs_preprocess=image_syn_preprocess, syn_labels=label_syn_sam, expert_model=Temp_net, target_layer=targetlayer)
        torch.save(gen_cams, os.path.join(mask_dir, "gen_cams.pth"))
    else:
        gen_cams = torch.load(os.path.join(mask_dir, "gen_cams.pth"))
        print("--> cams already exist, loaded")
    
    fore_masks = select_masks(all_masks, gen_cams, sel_p=args.sel_p)
    
    del Temp_net
    fore_masks = fore_masks.cuda()
    
    # visualize the masks
    if len(fore_masks) > 100:
        torchvision.utils.save_image(fore_masks[:100].detach().cpu().unsqueeze(1), os.path.join(save_dir, 'fore_mask.png'), nrow=10)
        torchvision.utils.save_image(image_syn_ori[:100].detach().cpu(), os.path.join(save_dir, 'image_syn_init.png'), nrow=10)
    else:
        torchvision.utils.save_image(fore_masks.detach().cpu().unsqueeze(1), os.path.join(save_dir, 'fore_mask.png'), nrow=10)
        torchvision.utils.save_image(image_syn_ori.detach().cpu(), os.path.join(save_dir, 'image_syn_init.png'), nrow=10)
    ##########################################################################################################################
    #                 finish generating foreground for synthetic images                                                      #
    ##########################################################################################################################   
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                # save_dir = os.path.join(args.save_dir, "logged_files", args.dataset, str(args.ipc), args.model, wandb.run.name)
                ...
                if args.ipc < 50 or args.force_save:
                    ...
                    grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=False, scale_each=False)
                    torchvision.utils.save_image(grid, os.path.join(save_dir, 'synthetic_image_{}.png'.format(it)))
                    ...
                    for clip_val in [2.5]:
                        ...
                        if args.dataset != "ImageNet":
                            ...
                            upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                        grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=False, scale_each=False)
                        torchvision.utils.save_image(grid, os.path.join(save_dir, 'cliped_synthetic_image_{}.png'.format(it)))
        ...
        # print("Synthetic_LR: {}".format(syn_lr.detach().cpu()))
        ...
        image_syn.grad *= fore_masks.unsqueeze(1).expand_as(image_syn)
        ...
        if it%10 == 0:
            print('%s iter = %04d, loss = %.4f, start_epoch = %2d, syn_lr = %.6f' % (get_time(), it, grand_loss.item(), start_epoch, syn_lr.detach().cpu()))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sel_p", type=float, default=0.7)
    ...
    logs_dir = os.path.join(args.save_dir, "logs", "{}_res{}".format(args.subset, args.res), "ipc{}".format(args.ipc))

    if not os.path.exists(logs_dir):
        os.makedirs(logs_dir, exist_ok=True)

    sys.stdout = Logger(os.path.join(logs_dir, 'selp{}_min{}_max{}.log'.format(args.sel_p, args.min_start_epoch, args.max_start_epoch)))
    main(args)
